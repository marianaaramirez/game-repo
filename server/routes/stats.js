...
/**
 * Computes the full aggregated stats object for one player.
 * Shared by GET /api/stats (self) and the admin per-player stats endpoint,
 * so both return the exact shape the StatsScene "My Stats" view consumes.
...
router.get('/stats', auth, async (req, res) => {
  try {
    return res.json(await computePlayerStats(req.player.playerID));
  } catch (err) {
